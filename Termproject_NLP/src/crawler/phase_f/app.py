...
def answer_fn(query: str):
    ...
    cat_badge = ""
    if res.predicted_category_name:
        cat_badge = f"  ·  📂 {res.predicted_category_name} (conf={res.classifier_confidence:.2f})"
    meta = (
        f"{fb_badge}{cat_badge}  ·  top rerank score: {res.top_rerank_score:+.3f}  ·  "
