...
case "$DAY" in
  ...
  all)
    ...
    python -m scripts.sprint2.dstat spike-alimi
    python -m scripts.sprint2.dept_grad discover --dept-list scripts/sprint2_dept_list.json
    for d in day1 day2 day3; do python -m scripts.sprint2.runner "$d"; done
    python -m scripts.sprint2.dept_grad crawl --candidates data/sprint2/day1/dept_grad_candidates.jsonl
    python -m scripts.sprint2.cross_tag
    python -m scripts.sprint2.faq_seed
    python -m scripts.sprint2.dorm_js
    for d in day1 day2 day3; do python -m scripts.sprint2.process_attachments "$d" --hwp-prefer hwp5txt --max 80; done
    python -m scripts.sprint2.verify
    ;;
  day1|day2|day3)
    python -m scripts.sprint2.runner "$DAY"
    case "$DAY" in
      day1) python -m scripts.sprint2.dept_grad crawl --candidates data/sprint2/day1/dept_grad_candidates.jsonl ;;
      day3) python -m scripts.sprint2.cross_tag; python -m scripts.sprint2.faq_seed; python -m scripts.sprint2.dorm_js ;;
    esac
    python -m scripts.sprint2.process_attachments "$DAY" --hwp-prefer hwp5txt --max 80
    python -m scripts.sprint2.verify
    ;;
  *)
