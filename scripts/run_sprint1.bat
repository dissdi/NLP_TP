...
if /I "!DAY!"=="attachments" (
    for %%d in (day1 day2 day4) do (
        echo === %%d attachments ===
        python -m scripts.sprint1_process_attachments %%d --hwp-prefer hwp5txt --max 80
    )
    python -m scripts.sprint1_verify
    exit /b %ERRORLEVEL%
)
...
if /I "!DAY!"=="all" (
    ...
    echo === !DAY! ===
    python -m scripts.sprint1_runner !DAY!
    if /I "!DAY!"=="day1" (
        python -m scripts.sprint1_process_attachments day1 --hwp-prefer hwp5txt --max 50
    )
    if /I "!DAY!"=="day2" (
        python -m scripts.sprint1_process_attachments day2 --hwp-prefer hwp5txt --max 100
    )
    if /I "!DAY!"=="day4" (
        python -m scripts.sprint1_process_attachments day4 --hwp-prefer hwp5txt --max 30
    )
)
